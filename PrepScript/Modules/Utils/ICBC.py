...
METGRID_COMPLETION_MARKER = '.metgrid.complete'


def Remove_Ungrib_Files(batch_path, time_str):
    """Remove Ungrib products from one completed time-batch directory."""
    removed_counts = {}
    ...
    logger.info(
        f"Removed Ungrib files for {time_str}: "
        f"2D={removed_counts['2D']}, 3D={removed_counts['3D']}, "
        f"SST={removed_counts['SST']}"
    )


def _Metgrid_Marker_Content(forcing_name, start_time, end_time):
    """Build deterministic content for a completed Metgrid batch marker."""
    time_str = f'{start_time.year}-{start_time.month:02d}-{start_time.day:02d}_{end_time.year}-{end_time.month:02d}-{end_time.day:02d}'
    return f'forcing={forcing_name}\ntime_range={time_str}\n'


def Write_Metgrid_Completion_Marker(batch_path, forcing_name, start_time, end_time):
    """Mark a batch complete after Metgrid and SST processing succeed."""
    marker_path = os.path.join(batch_path, METGRID_COMPLETION_MARKER)
    marker_content = _Metgrid_Marker_Content(forcing_name, start_time, end_time)
    with open(marker_path, 'w', encoding='utf-8') as marker_file:
        marker_file.write(marker_content)


def Check_Metgrid_Completion_Marker(batch_path, forcing_name, start_time, end_time, level='error'):
    """Check the completion marker for one Metgrid time batch."""
    marker_path = os.path.join(batch_path, METGRID_COMPLETION_MARKER)
    if not Tools.File_Exist(marker_path, level=level):
        return False

    expected = _Metgrid_Marker_Content(forcing_name, start_time, end_time)
    if not os.access(marker_path, os.R_OK):
        if level == 'error':
            raise PermissionError(f'Unable to read Metgrid completion marker: {marker_path}')
        logger.debug(f'Unable to read Metgrid completion marker: {marker_path}')
        return False
    with open(marker_path, 'r', encoding='utf-8') as marker_file:
        actual = marker_file.read()

    if actual == expected:
        return True

    message = f'Metgrid completion marker does not match: {marker_path}'
    if level == 'error':
...
def Check_Metgrid_Batch_Finish(casecfg, envcfg, gridname, start_time, end_time, level='error'):
    """Check one Metgrid batch before deciding whether it can be reused."""
    CaseOutputPath = casecfg.get(gridname, 'CaseOutputPath')
    ForcingDataName = casecfg.get(gridname, 'ForcingDataName').strip().lower()
    DataCalender = envcfg.get(ForcingDataName, 'DataCalender')
    time_str = f'{start_time.year}-{start_time.month:02d}-{start_time.day:02d}_{end_time.year}-{end_time.month:02d}-{end_time.day:02d}'
    batch_path = f'{CaseOutputPath}/{gridname}/PrepCWRF/Second_ICBC/UngribMetgrid_{time_str}'
    if not Check_Metgrid_Completion_Marker(batch_path, ForcingDataName, start_time, end_time, level=level):
        return False

    timeseries = Tools.Generate_Timeseries(start_time, end_time, DataCalender, freq='6H')
    return Tools.Check_Metgrid_Finish(batch_path, 'met_em.d01', timeseries, level=level)
...
def Metgrid(casecfg, envcfg, gridname, start_time, end_time):
    ...
    Clean_Ungrib = casecfg.getboolean('BaseInfo', 'Clean_Ungrib')
    ForcingDataName = casecfg.get(gridname, 'ForcingDataName')
    ForcingDataName = ForcingDataName.strip().lower()
    Forc_Info = envcfg.get(ForcingDataName, 'Forc_Info')
    CWRFCoreNum = casecfg.getint('PrepCWRF', 'CWRFCoreNum')
    ...
    #---------------------- Metgrid ----------------------
    if Go_Metgrid:
        marker_path = os.path.join(now_dir, METGRID_COMPLETION_MARKER)
        ...
        timeseries = Tools.Generate_Timeseries(start_time, end_time, DataCalender, freq='6H')
        Tools.Check_Metgrid_Finish(now_dir, 'met_em.d01', timeseries)
        ...
        Tools.Check_Metgrid_Finish(now_dir, 'met_em.d01', timeseries, level='error')
        Write_Metgrid_Completion_Marker(now_dir, ForcingDataName, start_time, end_time)
        if Clean_Ungrib:
            Remove_Ungrib_Files(now_dir, time_str)

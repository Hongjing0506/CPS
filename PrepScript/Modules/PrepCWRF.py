...
def First_StaticData(casecfg, envcfg, gridname):
    ...
    if (Go_Geogrid) or (Go_FVC) or (Go_LAI) or (Go_SAI) or (Go_IGBP) or (Collect_GeogData):
        logger.info(
            f"The steps are: ●  Create Geogrid -> Create FVC -> Geog-Post-Process -> "
            f"Create LAI -> Create IGBP -> Create SAI -> Collect Data ●\n"
        )
        if Go_Geogrid:
            ...
            # link global lake status data
            Tools.Link(
                f'{LandSeaMaskPath}/land_ocean_mask_igbp_2020.nc',
                f'{CaseOutputPath}/{gridname}/PrepCWRF/First_StaticData/Geogrid/land_ocean_mask_igbp_2020.nc'
            )
        ...
        if Go_FVC:
            ...
            cmd = (
                f'conda run -n {chaomodisenv} --no-capture-output python -u MODIS2CWRF.py -hvs "{SinGridList}" '
                f'-YS {syears} -YE {eyears} > {log_file} 2>&1'
            )
            print(cmd)
            ...
            cmd = (
                f'conda run -n {xesmfenv} --no-capture-output python -u process_geo.py -res {dx_WE} '
                f'-lk {LakeThreshold} -cpu {CWRFCoreNum} >> {log_file} 2>&1'
            )
        ...
        if Go_LAI:
            if Use_CoLMLAI:
                ...
                for year in range(syears, eyears+1):
                    ...
                    cmd = (
                        f'conda run -n {chaomodisenv} --no-capture-output python -u MODIS2CWRF.py -hvs "{SinGridList}" '
                        f'-YS {year} -YE {year} > {log_file} 2>&1'
                    )
                    cmds.append(cmd)
...
def Second_ICBC(casecfg, envcfg, gridname):
    ...
    TimeChunkGroupSize = casecfg.getint('BaseInfo', 'TimeChunkGroupSize')
    ...
    Reuse_Metgrid = casecfg.getboolean('BaseInfo', 'Reuse_Metgrid')
    StartTime = datetime.strptime(StartTime, '%Y-%m-%d_%H:%M:%S')
    EndTime = datetime.strptime(EndTime, '%Y-%m-%d_%H:%M:%S')
    ...
    if (Go_Ungrib) or (Go_Metgrid) or (Go_Real) or (Go_VBS):
        ...
        if Enable_TimeChunk: # slice process the whole period
            ...
            for i in range(TimeChunkCount):
                ...
                ICBCArgs.append((casecfg, envcfg, gridname, start_time, end_time))

            for group_start in range(0, len(ICBCArgs), TimeChunkGroupSize):
                group_end = min(group_start + TimeChunkGroupSize, len(ICBCArgs))
                group_args = ICBCArgs[group_start:group_end]
                runnable_args = []

                for icbc_args in group_args:
                    _, _, _, start_time, end_time = icbc_args
                    if Reuse_Metgrid and ICBC.Check_Metgrid_Batch_Finish(casecfg, envcfg, gridname, start_time, end_time, level='debug'):
                        time_str = (
                            f'{start_time.year}-{start_time.month:02d}-{start_time.day:02d}_'
                            f'{end_time.year}-{end_time.month:02d}-{end_time.day:02d}'
                        )
                        logger.info(f'{Consts.S4}Reuse complete Metgrid batch {time_str}; skip Ungrib and Metgrid.')
                        continue

                    ICBC.Link_CWPS_Files(casecfg, envcfg, gridname, start_time, end_time)
                    runnable_args.append(icbc_args)

                if not runnable_args:
                    logger.info(f'{Consts.S4}All ICBC batches in group {group_start // TimeChunkGroupSize + 1} were reused.')
                    continue

                Workers = len(runnable_args)
                logger.info(
                    f'{Consts.S4}!!! Processing ICBC group '
                    f'{group_start // TimeChunkGroupSize + 1}: chunks '
                    f'{group_start + 1}-{group_end} with {Workers} workers !!!'
                )

                # --- Ungrib ---
                Tools.Run_Parallel(ICBC.Ungrib, runnable_args, Workers, "Ungrib")
                logger.info(f"{Consts.S4}✦  Ungrib group Complete!")

                # --- Metgrid ---
                Tools.Run_Parallel(ICBC.Metgrid, runnable_args, Workers, "Metgrid")
                logger.info(f"{Consts.S4}✦  Metgrid group Complete!")

            logger.info(f"{Consts.S4}✦  All ICBC groups Complete or Reused!")
            ...
            can_reuse_metgrid = Reuse_Metgrid and ICBC.Check_Metgrid_Batch_Finish(casecfg, envcfg, gridname, start_time, end_time, level='debug')
            if can_reuse_metgrid:
                logger.info(f'{Consts.S4}Reuse complete Metgrid batch; skip Ungrib and Metgrid.')
            else:
                ICBC.Link_CWPS_Files(casecfg, envcfg, gridname, start_time, end_time)
                # --- Ungrib ---

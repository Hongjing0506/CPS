...
def First_GenMesh(casecfg, envcfg, gridname):
    ...
    if Go_MeshGrid:
        ...
        # link run script
        cmd = (
            f'conda run -n {cresmenv} --no-capture-output python -u GenCWRF2CoLMMesh.py '
            f'-dx {dx_WE} -dy {dy_SN} -reflat {RefLat} -reflon {RefLon} '
            f'-truelat1 {True_Lat1} -truelat2 {True_Lat2} -meshsize {MeshSize} '
            f'-geofile ./geo_em.d01_veg.nc -savename ./mesh_cwrf_{gridname}.nc'
        )
...
def Second_MakeSrf(casecfg, envcfg, gridname):
    ...
    if (Go_MakeSrf) :
        ...
        Tools.Copy(f'{ScriptPath}/Resources/NML/CoLM_Forcing/{ForcingName}.nml', f'./{ForcingName}.nml')
        Tools.Copy(f'{ScriptPath}/Resources/NML/history.colm.ctl', './history.nml')
        ...
        Tools.Copy(
            f'{CaseOutputPath}/{gridname}/PrepCoLM/Second_MakeSrf/unstructured_cwrf_{gridname}/landdata',
            f'{CaseOutputPath}/{gridname}/PrepCoLM/Second_MakeSrf/CoLMSrf_{gridname}/'
        )
        ...
        Tools.Copy(
            f'{CaseOutputPath}/{gridname}/PrepCoLM/First_GenMesh/mesh_cwrf_{gridname}.nc',
            f'{CaseOutputPath}/{gridname}/PrepCoLM/Second_MakeSrf/CoLMSrf_{gridname}/'
        )
        ...
        Tools.Copy(
            f'{CaseOutputPath}/{gridname}/PrepCoLM/Second_MakeSrf/CoLMSrf_{gridname}',
            f'{CaseOutputPath}/{gridname}/PrepCoLM/{gridname}/CoLMSrf_{gridname}'
        )
...
def Third_Remap(casecfg, envcfg, gridname):
    ...
    if Go_Remap:
        ...
        remap_pattern = (
            f'{CaseOutputPath}/{gridname}/PrepCoLM/Third_Remap/'
            f'unstructured_cwrf_{gridname}/history/'
            f'unstructured_cwrf_{gridname}_hist_{StartTime.year}_remap.nc'
        )
        remapfiles = glob.glob(remap_pattern)
        if len(remapfiles) == 0:
            ...
            raise FileNotFoundError(
                f'{Consts.S4}unstructured_cwrf_{gridname}/history/'
                f'unstructured_cwrf_{gridname}_hist_{StartTime.year}_remap.nc not found!'
            )

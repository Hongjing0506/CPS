...
#Modify: Haoran; 20200426 for MPI(CMPI6) ICBC
#Modify: Haoran; 20200505 for MPI(CMPI6) US ICBC
#Modify: Haoran; 20201112 for MPI(CMPI6) US ICBC （no leap year）
#Modify: Haoran; 20210312 for MPI(CMPI6) CN ICBC （no leap year）
...
monend=$6
dayend=$7
yrusestr=`printf "%04d" $yruse`
yrstastr=`printf "%04d" $yrsta`
yrendstr=`printf "%04d" $yrend`
monstastr=`printf "%02d" $monsta`
monendstr=`printf "%02d" $monend`
daystastr=`printf "%02d" $daysta`
dayendstr=`printf "%02d" $dayend`
BASE_DIR=/share/home/dq048/oscar/future/ICBC_RUN/SSP245/MPI_REAL/${yrusestr}/
MPI_DIR=/share/home/dq048/oscar/future/ICBC_RUN/SSP245/MPI_IM/


CWPS_DIR=/share/home/dq048/CWRF/CWPS/CWRF-CWPS_master/
GEO_DIR=/share/home/dq048/CWRF/CWPS/CWPS_static/
NML_DIR=/share/home/dq048/oscar/future/mpi245/icbc_mpi245/
EXE_DIR=/share/home/dq048/CWRF/CWPS/exes/
# EXE_DIR=/share/home/dq048/CWRF/CWPS/parallel_noleap/exes/

start_time=${yrstastr}${monstastr}${daystastr}00
#start_time=${yr}022700
#start_time=${yr}100100
# end_time=${yr}123118
end_time=${yrendstr}${monendstr}${dayendstr}18
...
# #link MPI files into temporary dictionary
# echo "link needed MPI files into Basedir"
# for yr in `seq ${start_time:0:4} ${end_time:0:4}`;do
# cd ${MPI_DIR}${yr}
# for fname in `ls MPI:*`;do
...
# ln -sf ${MPI_DIR}$yr/${fname}  ${BASE_DIR}/
# fi
# if [ ${date1} -eq ${end_time} ]; then
# break
# fi
...
# cp ${NML_DIR}/namelist.wps.MPI   namelist.wps
...
# ./metgrid.exe &> metgrid.MPI.ls

# echo "===============>MPI met files process<=========================="
...
#vbs process
rm -f sbcs
ln -s /share/home/dq048/CWRF/CWPS/CWPS_static/sbcs ./sbcs
cat > vbs.input << EOF

...
init_time = "040100"  # 起报时间改为4月1日00时
# 输出目录
...
for init_date in timelist:
    ...
    # 计算预报结束日期（11月1日）
    end_date = datetime(int(year), 11, 1)
    # 计算预报总小时数（从4月1日00时到11月1日00时）
    total_hours = int((end_date - datetime(int(year), 4, 1)).total_seconds() / 3600)
    ...
    for fhr in range(0, total_hours + 1, 6):
        # 计算目标日期时间
        target_time = datetime(int(year), 4, 1) + timedelta(hours=fhr)
